...
class Client(object):
    ...
    def _request(self, method, request_path, params, cursor=False):
        ...
        response = None
        if method == c.GET:
            response = requests.get(url, headers=header, timeout=DEFAULT_REQUEST_TIMEOUT_S)
            print("response : ",response.text)
        elif method == c.POST:
            response = requests.post(url, data=body, headers=header, timeout=DEFAULT_REQUEST_TIMEOUT_S)
            print("response : ",response.text)
            #response = requests.post(url, json=body, headers=header)
        elif method == c.DELETE:
            response = requests.delete(url, headers=header, timeout=DEFAULT_REQUEST_TIMEOUT_S)
    ...
    def _get_timestamp(self):
        ...
        response = requests.get(url, timeout=DEFAULT_TIME_SYNC_TIMEOUT_S)
